...
object SettingDescriptions {
    val byName: Map<String, String> = mapOf(
        ...
        "guest_scheduler" to "Run guest threads as cooperative fibers on a few host threads instead of one OS thread each; experimental, needs a restart",
        "guest_scheduler_cpus" to "How many host threads the cooperative scheduler spreads the 360's 6 CPUs across; 6 is full parallelism, 1 puts every guest thread on one thread. Needs a restart",
        "guest_scheduler_quantum_us" to "How long a guest fiber runs before yielding to others on its thread; lower is fairer but switches more often. Needs a restart",
        "memexport_enable" to "Route shader memory-export draws through a guest-RAM buffer so the CPU can read them; needs a driver that can import guest memory, off is faster",
        "memexport_await_fences" to "Wait for the GPU to finish memory export before signalling a fence the guest reads; accurate but stalls, needs memexport",
