...
namespace xe {
    namespace vfs {
        ...
        class SAF_StfsDevice : public Device {
        public:
            const static uint32_t kBlockSize = 0x1000;

            SAF_StfsDevice(const std::string_view mount_path,
                           std::unique_ptr<DocumentFile> path,std::unique_ptr<DocumentFile> data_dir);
            ~SAF_StfsDevice() override;

            bool Initialize() override;

            bool is_read_only() const override {
                return header_.content_metadata.volume_type != XContentVolumeType::kStfs ||
                       header_.content_metadata.volume_descriptor.stfs.flags.bits.read_only_format;
            }
            ...
            uint32_t total_allocation_units() const override {
                if (header_.content_metadata.volume_type == XContentVolumeType::kStfs) {
                    return header_.content_metadata.volume_descriptor.stfs.total_block_count;
                }
        ...
        private:
            ...
            Error ReadHeaderAndVerify(int header_file_fd);

            Error ReadSVOD();
            Error ReadEntrySVOD(uint32_t sector, uint32_t ordinal,
                                SAF_StfsEntry* parent);
            ...
            std::unique_ptr<DocumentFile> data_dir_;

            std::map<size_t, int> files_;
