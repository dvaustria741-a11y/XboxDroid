...
enum class ReadbackResolveMode {
  ...
  kUma,       // Read the mapped shared-memory buffer directly, no device->host
              // copy (uma). Adreno cannot import guest RAM (no
              // external_memory_host), so upstream's zero-copy never engages
              // there; this is the equivalent for such devices.
  kUmaSync    // uma, but drain the GPU before each read so guest RAM is
              // coherent within the frame instead of trailing it (uma-sync).
};
