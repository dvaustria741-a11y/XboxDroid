...
bool VulkanRenderTargetCache::Resolve(
  ...
  if (resolve_info.copy_dest_extent_length) {
    ...
    // memory. Decided before the copy shader is picked, because it changes the
    // scale the shader is selected for.
    ...
    if (GetPath() == Path::kHostRenderTargets) {
      ...
      // Resolve straight to guest memory before falling back to dumping the
      // render targets owning the affected range to edram_buffer_.
