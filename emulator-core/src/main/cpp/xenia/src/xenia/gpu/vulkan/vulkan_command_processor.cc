  ...
  if (cvars::log_gpu_frame_time_breakdown) {
    ...
    if (!s.last_report_ns) {
      ...
      XELOGI(
          ...
          s.rb_uma_direct / f, s.rb_uma_first_use);
...
bool VulkanCommandProcessor::IssueCopy() {
  ...
  // uma mode: the shared-memory buffer is host-mapped, so the resolved bytes
  // can be read straight out of it with no staging copy. Upstream's zero-copy
  // needs VK_EXT_external_memory_host, which Adreno does not expose.
  ...
  // fast/all copy into an imported guest-RAM buffer, which needs
  // VK_EXT_external_memory_host - absent on Adreno, where that leaves readback
  // doing nothing at all. Where the buffer is host-mapped instead, run uma: it
  // reaches guest RAM the same way.
  if ((readback_mode == ReadbackResolveMode::kFast ||
       readback_mode == ReadbackResolveMode::kAll) &&
      resolve_host_buffer == VK_NULL_HANDLE &&
      shared_memory_->IsHostMapped() &&
      !texture_cache_->IsDrawResolutionScaled()) {
    static bool readback_promoted_logged = false;
    if (!readback_promoted_logged) {
      readback_promoted_logged = true;
      XELOGW(
          "readback_resolve fast/all need a guest-RAM buffer this device "
          "cannot import - using uma instead");
    }
    readback_mode = (readback_mode == ReadbackResolveMode::kAll &&
                     cvars::readback_resolve_sync)
                        ? ReadbackResolveMode::kUmaSync
  ...
  // imported.
  const bool uma_sync = readback_mode == ReadbackResolveMode::kUmaSync;
  const bool uma_direct =
      (readback_mode == ReadbackResolveMode::kUma || uma_sync) &&
      !texture_cache_->IsDrawResolutionScaled() &&
      shared_memory_->IsHostMapped();
  ...
  if (readback_mode != ReadbackResolveMode::kDisabled && written_length > 0 &&
    ...
    // upstream's zero_copy on devices that cannot import guest RAM.
    if (uma_direct) {
      ...
      bool do_read;
      if (!last_write || uma_sync) {
        if (!last_write) {
          ++vk_frame_sync_stats_.rb_uma_first_use;
        }
        // First resolve of a destination (guest RAM holds nothing yet), or
        // uma-sync (caller wants same-frame coherency): make the just-recorded
        // write host-visible and drain so it can be read now.
        ...
        // Read the previous resolve's bytes once that submission has retired,
        // so the CPU never races the GPU writing the same region. Cached
        // completed-submission value - no blocking poll (vkGetFenceStatus
