...
DEFINE_string(
    ...
    "RAM.\n"
    " uma: No copy - the CPU reads the host-mapped shared memory directly on\n"
    "      unified-memory GPUs (Adreno etc.); falls back to fast when the\n"
    "      buffer is not host-visible (default)\n"
    " uma-sync: uma, but drains the GPU before each read so guest RAM is\n"
    "      coherent within the frame - accurate, much slower\n"
...
ReadbackResolveMode ParseReadbackResolveMode(const std::string& mode) {
  ...
  } else if (mode == "uma-sync") {
    return ReadbackResolveMode::kUmaSync;
  } else if (mode == "fast") {
...
void CommandProcessor::SetReadbackResolveMode(ReadbackResolveMode mode) {
  ...
  switch (mode) {
    ...
    case ReadbackResolveMode::kUma:
      mode_str = "uma";
      break;
    case ReadbackResolveMode::kUmaSync:
      mode_str = "uma-sync";
      break;
    default:
      break;
