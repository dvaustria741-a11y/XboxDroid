...
bool VulkanSharedMemory::Initialize() {
  ...
  const bool needs_host_mapping =
      cvars::readback_resolve.rfind("uma", 0) == 0;  // uma and uma-sync
  ...
  if (buffer_ == VK_NULL_HANDLE) {
    ...
    // No is_uma gate: that tests whether EVERY device-local type is also
    // host-visible, which Adreno fails on its LAZILY_ALLOCATED type, silently
    // disabling uma readback. The per-type search below is the real test.
  ...
  // State the effective readback backing once, so a measurement can never be
  // taken against a path that is not actually running.
  XELOGI(
      "Shared memory: zero_copy={} host_buffer={} host_mapped={} "
      "(readback_resolve={})",
      zero_copy_, host_buffer_ != VK_NULL_HANDLE,
      host_mapped_data_ != nullptr, cvars::readback_resolve);
  if (!zero_copy_ && host_buffer_ == VK_NULL_HANDLE) {
    if (host_mapped_data_) {
      XELOGI(
          "Shared memory: no host import, so resolve readback uses the direct "
          "mapped read for every mode");
    } else {
      XELOGW(
          "Shared memory: no host import and no mapped buffer - resolve "
          "readback cannot run, games reading resolved data will misbehave");
    }
  }
